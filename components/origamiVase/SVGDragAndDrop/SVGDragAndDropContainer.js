...
export default function SVGContainer() {
  ...
  const [displayCreasePattern, setDisplayCreasePattern] = React.useState(
    defaultValues.displayCreasePattern
  );
  const [creasePatternPointsArray, setCreasePatternPointsArray] =
    React.useState(Object.keys(points).map((p) => points[p]));
  const [realTimeDisplay, setRealTimeDisplay] = React.useState(
    defaultValues.realTimeDisplay
  );
  ...
  const mapPointsToSVGPoints = () => {
    return Object.keys(points).map((pointKey) => (
      <SVGDragAndDrop
        key={`SVG: ${pointKey}`}
        coords={points[pointKey]}
        setCoords={updateValueInPointsObject}
      />
    ));
  };
  ...
  const generatePolylinePointsStringFromPointsObject = () => {
    const pointsString = Object.keys(points)
      .map((pointKey) => {
  ...
  const generatePolyLine = () => {
    return (
      <polyline
        points={generatePolylinePointsStringFromPointsObject()}
        fill="none"
        stroke="black"
        strokeWidth="2"
        strokeLinecap="round"
  ...
  return (
    <div className={styles.mainWrapper}>
      ...
      <div className={styles.controlPanel}>
        <div className={styles.description}>
          <div>
            ...
            <p>
              The box with the black line going through all the blue circles
              represents the cross section of a vase, by moving the blue circles
              around you change the shape of the crease pattern below (scroll
              down!). Unless you are an advanced user/familiar with{" "}
              <a href="http://rebecca.gieseking.us/2013/03/tutorial-designing-pleated-forms-2/">
                the maths behind this
              </a>
              , it is recommended to leave the pleat width, number, length &
              total width settings alone. If you've made a vase why not try
              printing it out and folding it?
            </p>
          </div>
          <div className={styles.SVGControls}>
            <button onClick={addPointToPointsObject}>Add Point</button>
            <button onClick={removeLastPointFromPointsObject}>
              Remove Last Point
            </button>
            <div className={styles.goreInputs}>
              <label htmlFor="gore-number-input">Pleat Number:</label>
              <input
                id="gore-number-input"
                type="number"
                onChange={(e) => setGoreNumber(parseInt(e.target.value))}
                value={goreNumber}
              ></input>
              ...
              <input
                id="gore-height-input"
                type="number"
                onChange={(e) => setGoreLength(e.target.value)}
                value={goreLength}
              ></input>
              <label htmlFor="total-width-input">Total Width:</label>
              <input
                id="total-width-input"
                type="number"
                onChange={(e) => setTotalWidth(e.target.value)}
                value={totalWidth}
                min={(maxcx * maxcx * Math.PI) / 100}
              ></input>
              ...
              >
                {displayCreasePattern ? "Hide" : "Display"} Crease Pattern
              </button>
              {displayCreasePattern && (
                <>
                  <label htmlFor="gore-width-input">
                    Toggle updating Crease Pattern in real time?
                  </label>
                  <input
                    id="gore-width-input"
                    type="checkbox"
              ...
              {!realTimeDisplay && displayCreasePattern && (
                <>
                  <label htmlFor="gore-width-input">
      ...
      <div className={styles.creasePatternDisplay}>
        {displayCreasePattern && (
          <SVGCreasePatternContainer
            componentRef={componentRef}
            pointsArray={
              realTimeDisplay ? pointsArray : creasePatternPointsArray
            }
            goreWidth={goreWidth}
