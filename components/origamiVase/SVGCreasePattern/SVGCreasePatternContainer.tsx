...
import styles from './CreasePattern.module.css'
import GoreStrip from './GoreStrip';

function SVGCreasePatternContainer(
  {pointsArray, goreWidth, goreNumber, componentRef} : {
  ...
  const makeOutline = () => {
    return (
      <>
        <line x1={0} y1={0} x2={0} y2={lengthAlongLine(pointsArray)} style={{stroke: 'black'}}/>
        <line x1={0} y1={0} x2={goreNumber * goreWidth} y2={0} style={{stroke: 'black'}}/>
        <line x1={goreNumber * goreWidth} y1={0} x2={goreNumber * goreWidth} y2={lengthAlongLine(pointsArray)} style={{stroke: 'black'}}/>
        <line x1={goreNumber * goreWidth} y1={lengthAlongLine(pointsArray)} x2={0} y2={lengthAlongLine(pointsArray)} style={{stroke: 'black'}}/>
      </>
    );
  }
  ...
  return (
    <div className={styles.svgWrapper}>
      <svg
        ...
        viewBox={getViewBox()}>
        <g>
          {Array(goreNumber? goreNumber : 30).fill(0).map((_, i) => makeGore(i))}
          {makeOutline()}
        </g>
      </svg>
    </div>
